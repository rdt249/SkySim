...
print "\n" + datfile
thresh=0.75 #threshold voltage (0.5 VDD) for determining rising clock edges
tcol=1     #column in vector corresponding to the timestamps
...
#     print len(line)
     ...
     for j in range(1,len(cvec)):        #loop through the clock vector
          ...
          if rise>=thresh and rise>=starttime:               #if the value is above the threshold
     ...
     jitter=jitter/1e-12                 #average jitter in ps
     std=std/1e-12                       #standard deviation of jitter in ps

     print "Jitter:\t" + str(jitter) + " ps +- " + str(std) + " ps\n"
